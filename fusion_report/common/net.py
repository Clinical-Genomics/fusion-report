...
class Net:
    ...
    def get_cosmic_token(params: Namespace):
        ...
        if params.cosmic_token is None and (
            params.cosmic_usr is not None or params.cosmic_passwd is not None
        ):
            ...
            try:
                with urllib.request.urlopen(url, context=ctx) as response:
                    ...
                    if (
                        ...
                        or (response.info()["Content-Length"] or 0)
    ...
    def get_fusiongdb(self, return_err: List[str]) -> None:
        ...
        pool_params = [
            (f'{Settings.FUSIONGDB["HOSTNAME"]}/{x}', True)
            for x in Settings.FUSIONGDB["FILES"]
        ]
    ...
    def get_mitelman(self, return_err: List[str]) -> None:
        ...
        try:
            ...
            with ZipFile(Settings.MITELMAN["FILE"], "r") as archive:
                files = [
                    x for x in archive.namelist() if "mitelman_db/MBCA.TXT.DATA" in x
                ]
                archive.extractall()
